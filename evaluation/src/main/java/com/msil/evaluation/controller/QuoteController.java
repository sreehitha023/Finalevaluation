...
public class QuoteController {
    ...
    public ApiResponse<String> importSymbols(@RequestParam("file") MultipartFile file) {
        log.info(StringConstants.Import_Symbol_Request);
        try {
            quoteService.importSymbolsFromExcel(file);
            return new ApiResponse<>((StringConstants.Success),StringConstants.Symbols_Import_Successfully);
        } catch (IOException e) {
            return new ApiResponse<>((ErrorConstants.Failure),ErrorConstants.Symbols_Import_Error+e.getMessage());
        }
    }
    ...
    public ApiResponse<List<Symbols>> getAllSymbols() {
        ...
        return new ApiResponse<>((StringConstants.LIST_SYMBOLS_SUCCESS) ,quoteService.getAllSymbols());
    }
